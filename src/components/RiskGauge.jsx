...
export default function RiskGauge({ score = 0, size = 180 }) {
  ...
  const label = clamped < 34 ? 'LOW RISK' : clamped < 67 ? 'MODERATE RISK' : 'HIGH RISK'
  ...
  return (
    <div className="flex flex-col items-center" role="img" aria-label={`Risk score ${clamped} out of 100, ${label}`}>
